...
{ 
    ...
    public function getTeleworksUser()
    {
        ...
        $teleworks = Telework::where([
            ...
            ])->with(['user' => fn($query) => $query->where([['status', '=', "active"]])
            ])->get();
    ...
    {
        $tel = DB::table('teleworks')
        ->leftJoin('users', 'users.id', '=', 'teleworks.user_id')
        ->select('users.*','teleworks.*')
        ->where([
            ['teleworks.id', '=', $id],
        ])->update(['teleworks.is_deleted' =>1]);
        return response()->json([
            'telework' => $tel,
            'success' => true
        ], 200);
    ...
    public function acceptTelChefDep($id)
    {
        ...
        $telHistory = new teleworkHistory();
        $telHistory->is_rejected_prov = 0;
        $telHistory->level = 2;
        $telHistory->is_archive = 0;
        $telHistory->status = "Accepter";
        $telHistory->telework_id = $id;
        $telHistory->id_responsible = $this->user['id'];
        $telHistory->save();

        $telework->status="En cours";
        $telework->save();
       

        $test=Telework::where([['status','=','Envoyé'],['level','=','2'],['id','=',$id]])->orWhere([['status','=','En cours'],['level','=','2'],['id','=',$id]])
        ->with(['histories' => fn($query) => $query->where([['is_rejected_prov', '=', 0],['level', '=', 2],['status','=','Accepter'],['telework_id', '=', $id]]),
        ])->get();

        if(count($test) > 0){
            foreach ($test as $telework) {
    ...
    public function acceptTelGerant($id)
    {
        ...
        $telHistory->status = "Accepter";
        $telHistory->telework_id = $id;
        $telHistory->id_responsible = $leader['id'];
        $telHistory->save();
        ...
        $test=Telework::where([['status','=','En cours'],['level','=','3'],['id','=',$id]])
        ->with(['histories' => fn($query) => $query->where([['is_rejected_prov', '=', 0],['level', '=', 3],['status','=','Accepter'],['telework_id', '=', $id]]),
        ])->get();
    ...
    public function acceptTelLeader($id_telework)
    {
        ...
        if(in_array($leader['id'],$ids_leaders)){
            ...
            $telHistory->status = "Accepter";
            $telHistory->is_archive = 0;
            $telHistory->telework_id = $id_telework;
            $telHistory->id_responsible = $leader['id'];
            $telHistory->save();
            ...
            $test=Telework::where([['status','=','Envoyé'],['level','=','1'],['id','=',$id_telework]])->orWhere([['status','=','En cours'],['level','=','1'],['id','=',$id_telework]])->with([
    ...
    {
       $userAuth=teleworkController::getUser($id_user);
       $leader=teleworkController::getLeader($id_user);
       $chefDep=teleworkController::getChiefDepartement($id_user);
       $gerants=teleworkController::getAllGerants();
       $fonction_User_Auth=teleworkController::test_Leader_ChefDep_Gerant($id_user);
    ...
    public function RejetDefinitive(Request $request,$id_telework)
    {
        ...
        $telework = Telework::findOrFail($id_telework);
        $telework->status = "Rejet définitif";
        $telework->save();

        $telework_history = new teleworkHistory();
        $telework_history->id_responsible = $id_respons;
        $telework_history->status = "Rejet définitif";
        $telework_history->is_archive = 0;
        $telework_history->is_rejected_prov = 0;
        $telework_history->raison_reject = $request->raison_reject;
        $telework_history->level = $telework->level;
        $telework_history->telework_id = $id_telework;
        $telework_history->save();
    ...
    {    
        ...
        $teleworks = Telework::where([['user_id', '=', $id],['status', '=', "Rejet définitif"]])
                                 ...
                                 ->orwhere([['user_id', '=', $id],['status', '=', "Annuler"]])
                                    ->with(['histories' => fn($query) => $query->where([['id_responsible', '!=', $id]]),
                         ])->get();
    ...
    public function getAllTeleworksHistories($id_auth)
    {
        ...
        return response()->json([
            'teleworks' => array_values(array_unique($result)),
            'success' => true
        ], 200);
    }
